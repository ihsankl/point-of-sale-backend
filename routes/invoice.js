...
router.post("/", auth, async (req, res, next) => {
    const { total_amount, amount_tendered, date_recorded, user_id, customer_id } = req.body;
    const query = knex("Invoice").insert({ total_amount, amount_tendered, date_recorded, user_id, customer_id });
...
router.get("/", auth, async (req, res, next) => {
    const query = knex("Invoice").select("*");
    const result = await helper.knexQuery(query);
    res.status(result.status).send(result);
});
...
router.get("/:id", auth, async (req, res, next) => {
    const { id } = req.params;
    const query = knex("Invoice").where({ id });
    const result = await helper.knexQuery(query);
    res.status(result.status).send(result);
});
...
router.put("/:id", auth, async (req, res, next) => {
    const { id } = req.params;
    const { total_amount, amount_tendered, date_recorded, user_id, customer_id } = req.body;
    const query = knex("Invoice").where({ id }).update({ total_amount, amount_tendered, date_recorded, user_id, customer_id });
    const result = await helper.knexQuery(query);
    res.status(result.status).send(result);
});
