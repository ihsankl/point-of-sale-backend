...
const handleData = async (query, key) => {
  ...
  // if so, update redis and return data from database
  // if not, return data from redis
  const redisRawData = await redisClient.get(key);
  const result = await query;
  const redisData = JSON.parse(redisRawData);
  if (redisData !== result) return redisData;
  await redisClient
